...
from mm_std import FIREFOX_USER_AGENT, ahr, async_hrequest, json_dumps
...
async def test_custom_user_agent(httpserver: HTTPServer):
    ...
    user_agent = "moon cat"
    res = await async_hrequest(httpserver.url_for("/test"), user_agent=user_agent)
    assert res.json["user-agent"] == user_agent


async def test_json_parse_error():
    res = await async_hrequest("https://httpbin.org")
...
async def test_firefox_user_agent(httpserver: HTTPServer):
    ...
    res = await async_hrequest(httpserver.url_for("/user-agent"))
...
async def test_get_params(httpserver: HTTPServer):
    ...
    res = await async_hrequest(httpserver.url_for("/test"), params=data)
    assert res.json == data
...
async def test_timeout():
    res = await async_hrequest("https://httpbin.org/delay/10", timeout=2)
    assert res.error == "timeout"
    assert res.is_timeout_error()


async def test_proxy_error():
    res = await async_hrequest("https://httpbin.org/ip", proxy="https://google.com")
    assert res.error == "proxy_error"
    assert res.is_proxy_error()


async def test_connection_error():
    res = await async_hrequest("https://httpbin222.org/ip", timeout=2)
...
async def test_to_ok_result(httpserver: HTTPServer):
    ...
    res = await async_hrequest(httpserver.url_for("test"))
...
async def test_to_error():
    res = await async_hrequest("https://httpbin222.org/ip")
...
async def test_proxy():
    ...
    res = await ahr("https://api.ipify.org?format=json", proxy=proxy_url, timeout=5)
...
async def test_http_response_content_type(httpserver: HTTPServer):
    ...
    res = await async_hrequest(httpserver.url_for("test1"))
    assert res.content_type == "application/json"

    res = await async_hrequest(httpserver.url_for("test2"))
